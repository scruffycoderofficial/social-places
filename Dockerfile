...
COPY ./build/$BUILD_ARGUMENT_ENV/www.conf /usr/local/etc/php-fpm.d/www.conf
COPY ./build/$BUILD_ARGUMENT_ENV/php.ini /usr/local/etc/php/php.ini

# install Xdebug in case dev/test environment
COPY ./build/general/do_we_need_xdebug.sh /tmp/
COPY ./build/dev/xdebug.ini /tmp/
RUN chmod u+x /tmp/do_we_need_xdebug.sh && /tmp/do_we_need_xdebug.sh

# install security-checker in case dev/test environment
COPY ./build/general/do_we_need_security-checker.sh /tmp/
...
COPY ./build/general/supervisord.conf /etc/supervisor/conf.d/supervisord.conf
COPY ./build/general/cron /var/spool/cron/crontabs/root
